    ...
    return _first_non_empty(
        [
            _extract_labeled_value(
                text, ("degree", "qualification", "qualifications", "eligible degree", "educational qualification")
            ),
...
def extract_eligibility(text: str) -> dict[str, object]:
    ...
    batch_match = _first_non_empty(
        [
            _clean_field(match.group(1)) if (match := re.search(pattern, text, flags=re.IGNORECASE)) else None
    ...
    for line in text.splitlines():
        if any(keyword in line.lower() for keyword in branch_keywords):
            match = re.search(
                r"(?:branch(?:es)?|eligible branches|stream(?:s)?)\s*[:\-]?\s*([^\n.]+)", line, flags=re.IGNORECASE
            )
            if match:
    ...
    if branch_match:
        branches = [
            part.strip(" .;,") for part in re.split(r"\s*(?:,|/|&|and|or)\s*", branch_match) if part.strip(" .;,")
        ]
    else:
...
def analyze_text(text: str) -> dict[str, object]:
    ...
    return {
        ...
        "has_deadline_signal": bool(
            deadline_candidates or re.search(r"\b(due|deadline|submit|application)\b", normalized_text, re.IGNORECASE)
        ),
    }
